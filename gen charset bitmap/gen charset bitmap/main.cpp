...
void printChar(uint8_t c, EasyBMP::Image &img, int startPosX, int startPosY)
{
	int y = 0;
	for (auto line : _charset[c])
	{
		int x = 0;
		for (int i = 4; i >= 0; --i)
		{
			if (line & (1 << i))
			{
				img.SetPixel(startPosX + x, startPosY + y, colorCharset);
			}
			else
			{
				img.SetPixel(startPosX + x, startPosY + y, colorCharBackground);
			}
			x++;
		}
		y++;
...
int main()
{
	...
	int sizeCharcaterX = 5;
	int sizeCharacterY = 11;

	int numberOfCharsX = 16;
	int numberOfCharsY = 16;
	
	int spaceBottomChars = 1;
	int spaceRightChars = 1;
	...
	for (int x = 0; x < numberOfCharsX; x++)
	{
		for (int y = 0; y < numberOfCharsY; y++)
		{
